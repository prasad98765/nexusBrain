...
import { X, Plus, Trash2, Filter, AlertCircle, PlayCircle, Check, Layers, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function ConditionConfigDrawer({
    ...
    const getDropdownPosition = (groupId: string, conditionId: string) => {
        const key = `${groupId}-${conditionId}`;
        const element = dropdownRefs.current[key];
        if (!element) return { top: 0, left: 0, width: 'auto', openUpward: false };
                    ...
                    <div className="space-y-4">
                        {conditionGroups.map((group, groupIdx) => {
                            ...
                            return (
                                <div
                                    key={group.id}
                                    className="bg-slate-800/40 rounded-lg border-2 border-slate-700 transition-all"
                                >
                                    {/* Group Header with Collapse Toggle */}
                                    <div className="flex items-center justify-between p-4 bg-slate-800/60">
                                        ...
                                        <div className="flex items-center gap-1">
                                            <Button
                                                ...
                                                disabled={totalConditions >= 10}
                                                className="h-7 px-2 text-xs hover:bg-green-900/20 hover:text-green-400 text-slate-400"
                                            >
                                            ...
                                            <Button
                                                ...
                                                className="h-7 w-7 p-0 hover:bg-red-900/20 hover:text-red-400 text-slate-400"
                                            >
                                    ...
                                    {!isCollapsed && (
                                        <div className="px-4 pb-4 pt-2 overflow-visible">
                                            {/* Conditions in Group */}
                                            <div className="space-y-2 pl-2 overflow-visible">
                                                            ...
                                                            <div className="space-y-1.5">
                                                                ...
                                                                >
                                                                    ...
                                                                    {openDropdowns[`${group.id}-${condition.id}`] && (() => {
                                                                        const position = getDropdownPosition(group.id, condition.id);
                                                                        return (
                                                                            <div
                                                                                className="fixed z-[9999] bg-slate-800 border border-slate-700 rounded-lg shadow-xl max-h-60 overflow-y-auto"
                                                                                style={{
                                                                                    top: `${position.top}px`,
